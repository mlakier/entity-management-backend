...
const { CustomerInvoiceLine } = require("../models");
...
router.get("/", async (req, res) => {
    try {
        const lines = await CustomerInvoiceLine.findAll();
        res.json(lines);
    } catch (error) {
...
router.get("/:id", async (req, res) => {
    try {
        const line = await CustomerInvoiceLine.findByPk(req.params.id);
...
router.post("/", async (req, res) => {
    try {
        const newLine = await CustomerInvoiceLine.create(req.body);
...
router.put("/:id", async (req, res) => {
    try {
        const [updated] = await CustomerInvoiceLine.update(req.body, { where: { line_id: req.params.id } });
...
router.delete("/:id", async (req, res) => {
    try {
        const deleted = await CustomerInvoiceLine.destroy({ where: { line_id: req.params.id } });
